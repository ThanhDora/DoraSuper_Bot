...
from dorasuper import BOT_NAME, app, scheduler
from dorasuper.core.decorator import permissions
from dorasuper.core.decorator.permissions import require_admin
from dorasuper.helper.emoji_fmt import EMOJI_FMT
from dorasuper.helper.localization import langdict, use_chat_lang
from dorasuper.vars import COMMAND_HANDLER, LOG_CHANNEL, TZ
...
reply_markup = InlineKeyboardMarkup(
    [[InlineKeyboardButton(text="Night mode", callback_data="nightmd")]]
)
    ...
    try:
        ...
        await app.send_message(
            ...
            langdict[getlang]["nightmodev2"]["nmd_off_not_admin"].format(
                chat_id=chat_id, bname=BOT_NAME, **EMOJI_FMT
            ),
        )
        ...
        try:
            await app.send_message(
                ...
                langdict[getlang]["nightmodev2"]["nmd_off_success"].format(
                    dt=tglsekarang(), close_at=close_at, **EMOJI_FMT
                ),
...
async def mute_chat(chat_id: int):
    ...
    try:
        ...
        await app.send_message(
            LOG_CHANNEL,
            langdict[getlang]["nightmodev2"]["nmd_on_err"].format(chat_id=chat_id, e=e, **EMOJI_FMT),
        )
    else:
        ...
        await app.send_message(
            ...
            langdict[getlang]["nightmodev2"]["nmd_on_success"].format(
                dt=tglsekarang(), open_at=open_at, **EMOJI_FMT
            ),
            reply_markup=reply_markup,
        )
...
async def nightmode_handler(self, msg, strings):
    ...
    if "-d" in msg.text:
        if scheduler.get_job(job_id=f"enable_nightmode_{chat_id}"):
            ...
            if not bool(scheduler.get_jobs()) and bool(scheduler.state):
                scheduler.shutdown()
            return await msg.reply_msg(strings("nmd_disabled").format(**EMOJI_FMT))
        return await msg.reply_msg(strings("nmd_not_enabled").format(**EMOJI_FMT))
    ...
    try:
        ...
        return await msg.reply_msg(strings("invalid_time_format").format(**EMOJI_FMT), del_in=6)
    lockdur = re.findall(r"-e=(\w+)", msg.text)
    lockdur = lockdur[0] if lockdur else "6h"
    lock_dur = extract_time(lockdur.lower())

    if not lock_dur:
        return await msg.reply_msg(strings("invalid_lockdur").format(**EMOJI_FMT), del_in=6)
    ...
    try:
        ...
        return await msg.reply_msg(strings("schedule_already_on").format(**EMOJI_FMT))
    await msg.reply_msg(
        strings("nmd_enable_success").format(
            st=start_timestamp.strftime("%H:%M:%S"), lockdur=lockdur, **EMOJI_FMT
        )
    )
    ...
    try:
        await q.answer(
            strings("nmd_cb").format(
                bname=c.me.first_name, **EMOJI_FMT
            ),
            show_alert=True,
            cache_time=10,
        )
