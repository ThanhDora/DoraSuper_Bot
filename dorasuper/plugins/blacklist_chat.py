...
async def save_filters(_, message):
    if len(message.command) < 2:
        return await _reply_safe(message, f"{E_WARN} Cách sử dụng:\n<code>/dsden [TỪ|CÂU]</code>")
    word = message.text.split(None, 1)[1].strip()
    if not word:
        return await _reply_safe(message, f"{E_WARN} Cách sử dụng:\n<code>/dsden [TỪ|CÂU]</code>")
    chat_id = message.chat.id
    await save_blacklist_filter(chat_id, word)
    await _reply_safe(message, f"{E_SUCCESS} <b>Đã cấm</b> <code>{word}</code>.")
...
async def get_filterss(_, message):
    ...
    if not data:
        await _reply_safe(message, f"{E_LIST} Không có từ nào bị cấm trong cuộc trò chuyện này.")
    else:
        msg = f"{E_LIST} <b>Danh sách từ bị cấm</b> trong {message.chat.title}:\n\n"
        for word in data:
            msg += f"• <code>{word}</code>\n"
        await _reply_safe(message, msg)
...
async def del_filter(_, message):
    ...
    if deleted:
        return await _reply_safe(message, f"{E_SUCCESS} <b>Đã cho phép</b> <code>{word}</code>.")
    await _reply_safe(message, f"{E_CROSS} Không có từ/câu cấm nào như vậy.")
    ...
    for word in list_of_filters:
        ...
        if re.search(pattern, text, flags=re.IGNORECASE):
            ...
            try:
                ...
                return await _reply_safe(message, f"{E_WARN} Vui lòng cấp quyền quản trị viên cho tôi để cấm người dùng.")
            ...
            try:
                ...
                await app.send_message(chat_id, _emoji_to_unicode(notify_text), parse_mode=ParseMode.HTML)