...
async def save_filters(_, message):
    if len(message.command) < 2:
        return await message.reply_text(f"{E_WARN} Cách sử dụng:\n<code>/dsden [TỪ|CÂU]</code>", parse_mode=ParseMode.HTML)
    word = message.text.split(None, 1)[1].strip()
    if not word:
        return await message.reply_text(f"{E_WARN} Cách sử dụng:\n<code>/dsden [TỪ|CÂU]</code>", parse_mode=ParseMode.HTML)
    chat_id = message.chat.id
    await save_blacklist_filter(chat_id, word)
    await message.reply_text(f"{E_SUCCESS} <b>Đã cấm</b> <code>{word}</code>.", parse_mode=ParseMode.HTML)
...
async def get_filterss(_, message):
    ...
    if not data:
        await message.reply_text(f"{E_LIST} Không có từ nào bị cấm trong cuộc trò chuyện này.")
    else:
...
async def del_filter(_, message):
    if len(message.command) < 2:
        return await message.reply_text(f"{E_WARN} Cách sử dụng:\n<code>/dstrang [TỪ|CÂU]</code>", parse_mode=ParseMode.HTML)
    word = message.text.split(None, 1)[1].strip()
    if not word:
        return await message.reply_text(f"{E_WARN} Cách sử dụng:\n<code>/dstrang [TỪ|CÂU]</code>", parse_mode=ParseMode.HTML)
    chat_id = message.chat.id
    ...
    if deleted:
        return await message.reply_text(f"{E_SUCCESS} <b>Đã cho phép</b> <code>{word}</code>.", parse_mode=ParseMode.HTML)
    await message.reply_text(f"{E_CROSS} Không có từ/câu cấm nào như vậy.")
        ...
        if re.search(pattern, text, flags=re.IGNORECASE):
            ...
            try:
                ...
                return await message.reply(
                    f"{E_WARN} Vui lòng cấp quyền quản trị viên cho tôi để cấm người dùng.",
                    quote=False,
                )
