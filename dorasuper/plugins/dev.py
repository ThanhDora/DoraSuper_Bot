...
from dorasuper import BOT_NAME, app, botStartTime, dorasuper_version, user
from dorasuper.core.decorator import new_task
from dorasuper.helper.eval_helper import format_exception, meval
from dorasuper.helper.functions import extract_user, extract_user_and_reason
...
/stop - Dừng bot hoàn toàn (chỉ SUDO)
...
async def _auto_del_msg(msg: Message, seconds: int = 8):
...
async def privacy_policy(self: Client, ctx: Message, strings):
    await ctx.reply_msg(strings("privacy_policy").format(botname=self.me.first_name, **EMOJI_FMT), message_effect_id=5104841245755180586 if ctx.chat.type.value == "private" else None)
...
async def log_file(_, ctx: Message, strings):
    ...
    if len(ctx.command) == 1:
            ...
            await ctx.reply_document(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                strings("cl_btn").format(**EMOJI_FMT_BTN).strip(),
        ...
        val = ctx.text.split()
        tail = await shell_exec(f"tail -n {val[1]} -v DoraLogs.txt")
        try:
...
async def shell_cmd(self: Client, ctx: Message, strings):
    if len(ctx.command) == 1:
        return await edit_or_reply(self, ctx, text=strings("no_cmd").format(**EMOJI_FMT))
    msg = (
        await ctx.edit_msg(strings("run_exec").format(**EMOJI_FMT))
        if not self.me.is_bot
        else await ctx.reply_msg(strings("run_exec").format(**EMOJI_FMT))
    )
    ...
    if len(shell) > 3000:
            ...
            await ctx.reply_document(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                text=strings("cl_btn").format(**EMOJI_FMT_BTN).strip(),
        ...
        await ctx.reply_msg(strings("no_reply").format(**EMOJI_FMT), del_in=5)
...
async def cmd_eval(self: Client, ctx: Message, strings) -> Optional[str]:
    if (ctx.command and len(ctx.command) == 1) or ctx.text == "app.run()":
        return await edit_or_reply(self, ctx, text=strings("no_eval").format(**EMOJI_FMT))
    status_message = (
        await ctx.edit_msg(strings("run_eval").format(**EMOJI_FMT))
        if not self.me.is_bot
        else await ctx.reply_msg(strings("run_eval").format(**EMOJI_FMT), quote=True)
    )
    code = (
    ...
    async def _eval() -> Tuple[str, Optional[str]]:
        ...
        try:
            ...
            return f"{E_WARN} Error while executing snippet\n\n", formatted_tb
