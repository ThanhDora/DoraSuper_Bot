...
from dorasuper.emoji import E_BELL, E_CLOCK, E_INFO, E_PIN, E_STAT, E_TIP, E_WARN, E_WAIT
...
async def current_view(message: Message):
    wait = await message.reply_text(f"{E_WAIT} Đang tải dữ liệu giá vàng...", parse_mode=enums.ParseMode.HTML)
    data = await fetch_json(f"{API_BASE}/sjc")
    if not data or "results" not in data:
        return await wait.edit_text(f"{E_WARN} Không thể lấy dữ liệu từ máy chủ.", parse_mode=enums.ParseMode.HTML)
    ...
    summary = (
        f"{E_CLOCK} <i>Cập nhật: <b>{updated_at_vn}</b></i>\n\n"
    ...
    text += (
        ...
        f"{E_PIN} <b>Giá chuẩn SJC (1 lượng)</b>:\n"
    ...
    await wait.edit_text(text, disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML)
...
async def history_view(message: Message, limit: int = 5):
    ...
    if not data or "history" not in data:
        return await wait.edit_text(f"{E_WARN} Không thể tải lịch sử từ máy chủ.", parse_mode=enums.ParseMode.HTML)

    history = data["history"]
    text = "📈 <b>Lịch sử 5 lần cập nhật gần nhất</b>\n"
    text += f"{E_INFO} Hiển thị theo thời gian cập nhật gốc của API.\n\n"
...
async def chart_view(message: Message, limit: int = 10):
    ...
    data = await fetch_json(f"{API_BASE}/history?limit={limit}")
    if not data or "history" not in data:
        return await wait.edit_text(f"{E_WARN} Không thể tải dữ liệu biểu đồ.", parse_mode=enums.ParseMode.HTML)
    ...
    if buf:
        ...
        await wait.edit_text(f"{E_WARN} Không thể tạo biểu đồ.", parse_mode=enums.ParseMode.HTML)
...
async def advice_view(message: Message, limit: int = 5):
    ...
    data = await fetch_json(f"{API_BASE}/history?limit={limit}")
    if not data or "history" not in data:
        return await wait.edit_text(f"{E_WARN} Không thể lấy dữ liệu phân tích.", parse_mode=enums.ParseMode.HTML)

    history = data["history"]
    decision, reasons = analyze_for_advice(history)

    intro = (
        f"{E_TIP} <b>Gợi ý (tham khảo, dựa trên dữ liệu 3–5 phiên gần nhất)</b>\n"
        "Lưu ý: Đây là gợi ý tham khảo, không phải tư vấn đầu tư.\n\n"
    )

    advice_text = f"{E_BELL} <b>KẾT LUẬN:</b> <b>{decision}</b>\n\n"
