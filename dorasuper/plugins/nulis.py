...
async def handwrite(client, message):
    if message.reply_to_message and message.reply_to_message.text:
        ...
        return await message.reply(
            f"{E_ERROR} Vui lòng trả lời tin nhắn hoặc viết nội dung sau lệnh để sử dụng."
        )
    nan = await message.reply_msg(f"{E_LOADING} Đang xử lý...")
    try:
        ...
        return await message.reply(f"{E_ERROR} {e}")
