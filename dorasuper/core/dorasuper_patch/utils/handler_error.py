        ...
        with contextlib.suppress(Exception):
            try:
                await m.reply_photo(
                    ...
                    caption=f"{E_ERROR} Đã xảy ra lỗi nội bộ trong khi xử lý lệnh của bạn, Nhật ký đã được gửi đến @dabeecao. Xin lỗi vì sự bất tiện",
                    parse_mode=enums.ParseMode.HTML,
                )
            except:
                await m.reply_msg(
                    f"{E_ERROR} Đã xảy ra lỗi nội bộ trong khi xử lý lệnh của bạn, Nhật ký đã được gửi đến @dabeecao. Xin lỗi vì sự bất tiện",
                    parse_mode=enums.ParseMode.HTML,
                    del_in=8,
                )
