...
# Chức năng tạm biệt (khi thành viên rời nhóm)
async def is_goodbye(chat_id: int) -> bool:
    data = await greetingdb.find_one({"chat_id": chat_id})
    return bool(data and data.get("goodbye_enabled", False))

async def toggle_goodbye(chat_id: int):
    data = await greetingdb.find_one({"chat_id": chat_id})
    if data and data.get("goodbye_enabled", False):
        await greetingdb.update_one(
            {"chat_id": chat_id},
...
async def set_custom_goodbye(chat_id: int, custom_message: str, buttons: list = None):
    await greetingdb.update_one(
        {"chat_id": chat_id},
        {"$set": {
            "goodbye_enabled": True,
            "custom_goodbye_message": custom_message,
            "goodbye_buttons": buttons or []
        }},
        upsert=True
    )

async def get_custom_goodbye(chat_id: int):
    data = await greetingdb.find_one({"chat_id": chat_id})
    if data and "custom_goodbye_message" in data:
        return data["custom_goodbye_message"], data.get("goodbye_buttons", [])
    return None, []
